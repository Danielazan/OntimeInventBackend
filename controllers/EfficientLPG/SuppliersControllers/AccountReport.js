const {
    ESupplierReport,
  } = require("../../../models/EfficientLPG/SuppliersModels/AccountReport");
  ...
  const CreateSupplierReport = async (req, res) => {
    ...
    try {
      const pro = await ESupplierReport.create({
         SupplierName,
          Credit,
          Debit
      }).then((result) => {
...
const GetAllTSupplierReport = async (req, res) => {
    try {
      const Cat = await ESupplierReport.findAll().then((result) => {
  ...
  const GetSingleSupplierReport = async (req, res) => {
    ...
    try {
      const Getone = await ESupplierReport.findOne({ where: { id: TramsferId } }).then(
        (result) => {
          res.status(200).json({ result });
        }
      );
  ...
  const DeleteSupplierReport = async (req, res) => {
    try {
      ...
      const Cat = await ESupplierReport.destroy({
        where: { id },
        cascade: true,
      }).then((result) => {
