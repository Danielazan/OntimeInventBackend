const {
    BankDepositTransfer,
  } = require("../../../models/Pillar Pole/AccountsModels/BankDepositTransfer");
  ...
  const CreateBankDepositTransfer = async (req, res) => {
    ...
    try {
      const pro = await BankDepositTransfer.create({
        PaidTo,
          Description,
          Amount,
          Date,
  ...
  const GetAllBankDepositTransfer = async (req, res) => {
    try {
      const Cat = await BankDepositTransfer.findAll().then((result) => {
  ...
  const GetSingleNewBankDepositTransfer = async (req, res) => {
    ...
    try {
      const Getone = await BankDepositTransfer.findOne({ where: { id: BankDepositTransferId } }).then(
        (result) => {
          res.status(200).json({ result });
        }
      );
  ...
  const DeleteNewBankDepositTransfer = async (req, res) => {
    try {
      ...
      const Cat = await BankDepositTransfer.destroy({
        where: { id },
        cascade: true,
      }).then((result) => {
