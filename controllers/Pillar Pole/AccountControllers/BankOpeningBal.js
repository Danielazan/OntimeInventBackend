const {
    BankOpeningBalance,
  } = require("../../../models/Pillar Pole/AccountsModels/BankOpeningBal");
  ...
  const CreateBankOpeningBalance = async (req, res) => {
    ...
    try {
      const pro = await BankOpeningBalance.create({
        BankAccount,
          OpeningBalance,
          Date,
          Location,
  ...
  const GetAllBankOpeningBalance = async (req, res) => {
    try {
      const Cat = await BankOpeningBalance.findAll().then((result) => {
  ...
  const GetSingleNewBankOpeningBalance = async (req, res) => {
    ...
    try {
      const Getone = await BankOpeningBalance.findOne({ where: { id: BankOpeningBalanceId } }).then(
        (result) => {
          res.status(200).json({ result });
        }
      );
  ...
  const DeleteNewBankOpeningBalance = async (req, res) => {
    try {
      ...
      const Cat = await BankOpeningBalance.destroy({
        where: { id },
        cascade: true,
      }).then((result) => {
