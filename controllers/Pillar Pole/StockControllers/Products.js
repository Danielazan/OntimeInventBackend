const { Product } = require("../../../models/Pillar Pole/StockModels/Product");
...
const GetAllProducts = async (req, res) => {
  try {
    const Cat = await Product.findAll().then((result) => {
...
const GetSingleProducts = async(req,res)=>{
  ...
  try {
    ...
    const Getone = await Product.findOne({where: {id:ProductId}}).then(result =>{
      res.status(200).json({result})
    })
  } catch (error) {
    res.status(400).json({error:error.message})
