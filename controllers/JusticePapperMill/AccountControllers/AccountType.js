const { JAccountType } = require("../../../models/JusticePapperMill/AccountsModels/AccountType");
...
const CreateAccountTypes = async (req, res) => {
  ...
  try {
    ...
    const pro = await JAccountType.create({
...
const GetAllAccountTypes = async (req, res) => {
  try {
    const Cat = await JAccountType.findAll().then((result) => {
...
const GetSingleAccountType = async(req,res)=>{
  ...
  try {
    ...
    const Getone = await JAccountType.findOne({where: {id:AccountTypeId}}).then(result =>{
      res.status(200).json({result})
    })
  } catch (error) {
    res.status(400).json({error:error.message})
...
const DeleteAccountType = async (req, res) => {
  try {
    ...
    const Cat = await JAccountType.destroy({
      where: { id },
      cascade: true,
    }).then((result) => {
