const {
    JSupplierReport,
  } = require("../../../models/JusticePapperMill/SupplierModels/SupplierReport");
  ...
  const CreateSupplierReport = async (req, res) => {
    ...
    try {
      const pro = await JSupplierReport.create({
         SupplierName,
          Credit,
          Debit
      }).then((result) => {
...
const GetAllTSupplierReport = async (req, res) => {
    try {
      const Cat = await JSupplierReport.findAll().then((result) => {
  ...
  const GetSingleSupplierReport = async (req, res) => {
    ...
    try {
      const Getone = await JSupplierReport.findOne({ where: { id: TramsferId } }).then(
        (result) => {
          res.status(200).json({ result });
        }
      );
