const {
    EBankOpeningBalance,
  } = require("../../../models/EfficientLPG/AccountModels/BankOpeningBal");
  ...
  const CreateBankOpeningBalance = async (req, res) => {
    ...
    try {
      const pro = await EBankOpeningBalance.create({
        BankAccount,
          OpeningBalance,
          Date,
          Location,
  ...
  const GetAllBankOpeningBalance = async (req, res) => {
    try {
      const Cat = await EBankOpeningBalance.findAll().then((result) => {
  ...
  const GetSingleNewBankOpeningBalance = async (req, res) => {
    ...
    try {
      const Getone = await EBankOpeningBalance.findOne({ where: { id: BankOpeningBalanceId } }).then(
        (result) => {
          res.status(200).json({ result });
        }
      );
  ...
  const DeleteNewBankOpeningBalance = async (req, res) => {
    try {
      ...
      const Cat = await EBankOpeningBalance.destroy({
        where: { id },
        cascade: true,
      }).then((result) => {
